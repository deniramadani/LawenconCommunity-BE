...
public class PaymentDao extends AbstractJpaDao{
	...
	public List<ReportResDto> getProductivitySuperAdminData(final String startDate, final String endDate) {
		...
		if(result != null && result.size() > 0) {
			result.forEach(objCol -> {
				...
				final ReportResDto row = new ReportResDto();
				row.setNo(Long.valueOf(objArr[0].toString()));
				row.setMemberId(objArr[1].toString());
				row.setMemberName(objArr[2].toString());
				row.setTotalParticipants(Integer.valueOf(objArr[3].toString()));
				data.add(row);
			});
		}
		return data;
	...
	@SuppressWarnings("unchecked")
	public List<Payment> getValAccepted(final String userId, final String productId)  {
		final StringBuilder query = new StringBuilder()
				.append("SELECT * ")
				.append("FROM tb_payment tp ")
				.append("INNER JOIN tb_user tu ON tu.id = tp.user_id ")
				...
				.append("INNER JOIN tb_file tfoto ON tfoto.id = p.photo_id ")
				.append("WHERE tp.user_id = :userId AND tp.product_id = :productId ")
				.append("AND tp.approval = true AND tp.is_active = true ")
				.append("ORDER BY tp.created_at DESC ");
		final List<Payment> result = ConnHandler.getManager().createNativeQuery(query.toString(), Payment.class)
				.setParameter("userId", userId).setParameter("productId", productId).getResultList();
		return result;
	...
	@SuppressWarnings("unchecked")
	public List<Payment> getValApproval(final String userId, final String productId)  {
		final StringBuilder query = new StringBuilder()
				.append("SELECT * ")
				.append("FROM tb_payment tp ")
				.append("INNER JOIN tb_user tu ON tu.id = tp.user_id ")
				...
				.append("INNER JOIN tb_file tfoto ON tfoto.id = p.photo_id ")
				.append("WHERE tp.user_id = :userId AND tp.product_id = :productId ")
				.append("AND tp.approval = false AND tp.is_active = true ")
				.append("ORDER BY tp.created_at DESC ");
		final List<Payment> result = ConnHandler.getManager().createNativeQuery(query.toString(), Payment.class)
				.setParameter("userId", userId).setParameter("productId", productId).getResultList();
		return result;
